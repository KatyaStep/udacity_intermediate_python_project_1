...
def make_parser():
    ...
    parser.add_argument('--neofile', default=(DATA_ROOT / 'test-neos-2020.csv'),
                        type=pathlib.Path,
                        help="Path to CSV file of near-Earth objects.")
    parser.add_argument('--cadfile', default=(DATA_ROOT / 'test-cad-2020.json'),
                        type=pathlib.Path,
...
def query(database, args):
    ...
    if not args.outfile:
        ...
        for result in limit(results, args.limit or 15):
            print(result)
    else:
